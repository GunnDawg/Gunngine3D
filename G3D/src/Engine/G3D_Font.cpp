	...
	void Font::Draw()
	{
		...
		if (Settings::General::DevMode)
		{
			...
			else if (Settings::General::ShowingLights)
			{
				...
				sprintf(Buffer2, "'F2' Hide Lighting Information\n\nStrength: %.02f", AmbientStr);

				spriteBatch->Begin();
					spriteFont->DrawString(spriteBatch.get(), Buffer2, DirectX::XMFLOAT2(0.0f, 0.0f), DirectX::Colors::White, 0.0f, DirectX::XMFLOAT2(0.0f, 0.0f), DirectX::XMFLOAT2(1.0f, 1.0f));
				spriteBatch->End();
			}
			else
			{
